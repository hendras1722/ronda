...
async function runStartTask(
  ...
  if (videoEl.srcObject !== undefined) {
    ...
    videoEl.src = (window.webkitURL.createObjectURL as CreateObjectURLCompat)(
      stream
    )
  } else {
  ...
  const capabilities: Partial<MediaTrackCapabilities> =
    track?.getCapabilities?.() ?? {}

  let isTorchOn = false
  if (torch && (capabilities as any).torch) {
  ...
  taskQueue = taskQueue
    .then((prevTaskResult) => {
      if (prevTaskResult.type === 'start') {
        ...
        prevTaskResult.type === 'stop' ||
        prevTaskResult.type === 'failed'
      ) {
    ...
    .catch((error: Error) => {
      console.debug(
        `[vue-qrcode-reader] starting camera failed with "${error}"`
      )
